...
QString generateHtmlReport(QSqlTableModel *model) {
    ...
    html += "<h2>Отчёт: Список на премию</h2>";
...
void exportPdf(QSqlTableModel *model) {
    ...
    QFontDatabase database;
       qInfo()<< database.families();

   QFile debugsFile("/Volumes/MacSSD/Users/admin/PROJECT/CPPQT/debugs.txt");
      debugsFile.open(QIODevice::WriteOnly | QIODevice::Text);
      debugsFile.write("njnkhbk\n");
      debugsFile.close();

    // Вывод в HTML файл для контроля PDF принтера
    if(true){
      QFile debugFile("/Volumes/MacSSD/Users/admin/PROJECT/CPPQT/debug.html");
...
int main(int argc, char *argv[]) {
    ...
    QPushButton *btnPdf = new QPushButton("Сохранить в PDF и ОтладФайлы");
